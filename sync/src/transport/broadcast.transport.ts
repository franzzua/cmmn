...
export abstract class DOMTransport<T extends Record<string, any>>
	extends EventEmitterBase<T>
	implements Transport<T>
{
	...
	public getChannel<TKey extends keyof T>(
		channel: TKey,
	): TransportChannel<T[TKey]> {
	...
	on<TEventName extends keyof T>(
		...
		listener: (data: T[TEventName]) => void,
		...rest
	): () => void {
		return this.getChannel(eventName).on('message', listener);
	}

	off<TEventName extends keyof T>(
		eventName: TEventName,
		listener: (data: T[TEventName]) => void,
	) {
...
export class EventTargetTransportChannel<T>
