export class Serializer {
	...
	write(value: SerializerType) {
		if (value instanceof Uint8Array) this.writeUint8array(value);
		else
			switch (value.size) {
				case 8:
	...
	static serialize(data: Array<SerializerType>) {
		const size = data.map(Serializer.typeSize).reduce((a, b) => a + b, 0);
	...
	static typeSize(data: SerializerType) {
		if (data instanceof Uint8Array) return data.length + 2;
		return data.size / 8;
	}
}

type SerializerType = Uint8Array | { value: number; size: 8 | 16 | 32 };
