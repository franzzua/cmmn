...
export class RpcClient {
    ...
    getFixed<T extends RpcService>(name: string, methods: Array<keyof T>): RPC<T> {
        return Object.fromEntries(methods.map(method => [
            method, (...args) => this.call(name, method as string, args)
        ])) as any as RPC<T>;
    }

    call<T = any>(service: string, method: string = 'get', args: any[] = []): Promise<T>  {
        const id = uuid();
    ...
    static proxy<T extends RpcService>(service: string): (c: Container) => RPC<T> {
        return c => c.resolve(RpcClient).getProxy(service);
    }

    static fixed<T extends RpcService>(service: string, keys: Array<keyof T>): (c: Container) => RPC<T> {
...
export type RPC<T extends RpcService> = {
    [key in keyof T]: T[key] extends (...args: infer TArgs) => infer TResult
        ? (...args: TArgs) => (TResult extends PromiseLike<any> ? TResult : Promise<TResult>)
        : T[key]
}