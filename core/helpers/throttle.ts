...
export function throttle<
	TResult,
	TThis = void,
	TNewArgs extends unknown[] = [],
	TArgs extends unknown[] = TNewArgs,
>(
	...
	const throttler = getThrottler<TResult, TThis, TNewArgs, TArgs>(
		wait,
		options,
	);
	return throttler(fn);
...
export type Func<TResult, TThis = void, TArgs extends unknown[] = []> = (
	this: TThis,
	...args: TArgs
) => TResult;
export type ThrottleFunc<
	TResult,
	TThis = void,
	TArgs extends unknown[] = [],
> = (this: TThis, ...args: TArgs) => TResult;

export type ThrottledFunc<
	TResult,
	TThis = void,
	TArgs extends unknown[] = [],
	...
	return (
		...
		const orig = throttleOrig(
			...
			async function () {
				...
				try {
					const args: TNewArgs[] = options.select
						? options.select.apply(this, allArgs)
						: allArgs.pop();
					allArgs = [];
		...
		return Object.assign(
			function (this: TThis, ...args: TArgs): Promise<Awaited<TResult>> {
				allArgs.push(args);
				orig.call(this);
