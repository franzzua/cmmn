...
export function SubcribeOnActions<TState>(component: HtmlComponentBase<TState>, actionFn, filter, subscribeAt){
    ...
    async function actionSubscribe(){
        ...
        component.once('dispose', cell.on('error', e => component.onError(e, 'action', actionFn.name)));
        component.once('dispose', cell.on('change', invokeAction));
    ...
    switch (subscribeAt){
        ...
        case ActionSubscribeType.OnFirstRender:
            component.on('dispose', component.once('render',  actionSubscribe));
            break;
    }
}
