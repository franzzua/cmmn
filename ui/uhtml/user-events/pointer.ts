...
export type RelativePointerEvent<TEvent = PointerEvent> = {
    event: TEvent;
    point: IPoint;
};
...
export class PointerListener extends EventListener<PointerEvents> {
    ...
    public getRelativePoint(
        event: MouseEvent | Touch | PointerEvent | GestureEvent,
    ): IPoint {
    ...
    public get Position(): RelativePointerEvent {
        if (this._position) return this._position.get();
    ...
    private async dragListener(downEvent: RelativePointerEvent) {
        ...
        if (downEvent.event.target !== this.root) return;
        if (!downEvent.event.isPrimary) return;
        ...
        this.once(
            ...
            (event) => {
                ...
                if (isStarted) {
                    (this.root as HTMLElement).releasePointerCapture(
                        downEvent.event.pointerId,
                    );
                    this.emit('drag', {
                        ...event,
                        shift: undefined,
    ...
    private async directClickListener(downEvent: RelativePointerEvent) {
        ...
        if (upEvent.event.timeStamp - downEvent.event.timeStamp > 400) return;
        if (Math.abs(upEvent.event.x - downEvent.event.x) > 1) return;
        if (Math.abs(upEvent.event.y - downEvent.event.y) > 1) return;
        this.emit('directClick', upEvent);
    }

    private emitters = {
        move: (event) =>
            this.emit('move', {event, point: this.getRelativePoint(event)}),
        ...
        down: (event) =>
            this.emit('down', {event, point: this.getRelativePoint(event)}),
        up: (event) =>
            this.emit('up', {event, point: this.getRelativePoint(event)}),
        enter: (event) =>
            this.emit('enter', {event, point: this.getRelativePoint(event)}),
        leave: (event) =>
            this.emit('leave', {event, point: this.getRelativePoint(event)}),
        click: (event) =>
            this.emit('click', {event, point: this.getRelativePoint(event)}),
        wheel: (event) =>
            this.emit('wheel', {event, point: this.getRelativePoint(event)}),
        dblclick: (event) =>
            this.emit('dblclick', {event, point: this.getRelativePoint(event)}),
    ...
    protected subscribe(eventName: keyof PointerEvents) {
        switch (eventName) {
            ...
            case 'wheel':
            case 'dblclick':
                this.root.addEventListener(eventName as any, this.emitters[eventName], {
                    passive: true,
                });
                break;
            case 'click':
