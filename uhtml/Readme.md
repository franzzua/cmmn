...
decorators can be generated by comman `cmmn gen my-comp ./components`

it will generate 3 files: `my-comp.decorators.ts`, `my-comp.template.ts` and `my-comp.style.less`
...
export class MyComponent extends HtmlComponent<IState, IEvents> {
    ...
    @action(() => version)
    otherAction(version) {
        // it will be called every time the version changes after decorators will be connected
    }
...
2. About templates you can read at [uhtml](https://github.com/WebReflection/uhtml), but
    1. html`...` renders inside current decorators
...
8. You can use `this.element.parentElement.decorators` for access to parent decorators, if parentElement is root of it.
